...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	if cfg.CodeMerkleization {
		...
		if stats.NumContracts > 0 {
			if _, err := cmFile.WriteString(fmt.Sprintf("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", block.NumberU64(), stats.CodeSize, stats.ProofSize, stats.RLPStats.RLPSize, stats.RLPStats.UnRLPSize, stats.RLPStats.SnappySize, stats.RLPStats.SnappySlim, stats.ProofStats.Indices, stats.ProofStats.ZeroLevels, stats.ProofStats.Hashes, stats.ProofStats.Leaves)); err != nil {
				return nil, nil, 0, err
			}
		}
